...
func (kl *Kubelet) handlePodResourcesResize(pod *v1.Pod) {
	...
	if fit, patchBytes := kl.canResizePod(pod); fit {
		tenantClient := kubeclientmanager.ClientManager.GetTPClient(kl.kubeTPClients, pod.Tenant)
...
func (kl *Kubelet) HandlePodActions(update kubetypes.PodUpdate) {
	...
	for _, action := range update.Actions {
		...
		if !exist {
			...
			tenantClient := kubeclientmanager.ClientManager.GetTPClient(kl.kubeTPClients, action.Tenant)
