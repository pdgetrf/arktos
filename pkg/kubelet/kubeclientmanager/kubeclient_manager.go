...
type KubeClientManager struct {
	tenant2api     map[string]int // tenant name -> api server id
...
func (manager *KubeClientManager) RegisterTenantSourceServer(source string, ref *v1.Pod) {
	...
	if source == "api" {
		klog.Warning("source is 'api', will only use kube client #0")
	...
	if err != nil {
		klog.Errorf("unable to get apiserver id, Err: %s", err)
		return
	}
